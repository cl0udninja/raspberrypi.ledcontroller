...
public class UpnpListener {
    ...
    try (DatagramSocket responseSocket = new DatagramSocket(upnpResponsePort);
      ...
      while (ifs.hasMoreElements()) {
        ...
        while (addrs.hasMoreElements()) {
          ...
          log.trace(name + " ... has addr " + addr);
          if (InetAddressUtils.isIPv4Address(addr.getHostAddress())) {
            IPsPerNic++;
          }
        }
        log.trace("Checking " + name + " to our interface set");
        if (IPsPerNic > 0) {
          upnpMulticastSocket.joinGroup(socketAddress, xface);
          log.trace("Adding " + name + " to our interface set");
        }
      }
  ...
  String discoveryTemplate = "HTTP/1.1 200 OK\r\n" + "CACHE-CONTROL: max-age=86400\r\n" + "EXT:\r\n"
      ...
      + "01-NLS: %s\r\n" + "ST: urn:schemas-upnp-org:device:RaspberryPiLedSwitch:1\r\n"
  ...
  protected void sendUpnpResponse(DatagramSocket socket, InetAddress requester, int sourcePort, int gatewayPort,
    ...
    log.trace(String.format("sendUpnpResponse %s %s", requester.getHostAddress(), sourcePort));
  ...
  protected String getRandomUUIDString() {
    return "6CD6EBCF-A2E9-4E04-80EA-5667D1150811"; // https://xkcd.com/221/
  }
}